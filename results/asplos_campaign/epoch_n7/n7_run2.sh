#!/bin/bash
# variante con tamanos de transferencia parametrizables
set -u
ET="$1"; ABL="${2:-full}"; NOPARK="${3:-0}"; IT="${4:-40}"; B1="${5:-8388608}"; B2="${6:-25165824}"
...
docker run --rm --name gv_n7 --network host --device /dev/infiniband \
  ...
  -c "ulimit -c 0; LD_PRELOAD=libcuda.so.1:libcudart.so.12 timeout 300 $RT/epochharm $IT $B1 $B2" \
  > "$OUT/$ET.log" 2>&1
echo "exit=$?"
