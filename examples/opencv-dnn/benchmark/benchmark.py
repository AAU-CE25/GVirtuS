...
if MODE not in {"tcp", "rdma", "ucx", "baremetal"}:
    print("Usage: ./benchmark.sh {tcp|rdma|ucx|baremetal}", file=sys.stderr)
    sys.exit(2)
...
FRONTEND_CMD = os.environ.get("FRONTEND_CMD", "./dnn_test")

# Baremetal: dnn_test runs INSIDE the opencv-yolo container with --gpus all,
# NO LD_PRELOAD (real libcudart/libcudnn from the system, not GVirtuS shims),
# NO GVIRTUS_* env. Inference goes straight to the local L40S of dpu-02.
# The host process spawned by run_one is the docker invocation itself —
# wall time includes ~1s of docker startup which is consistent with the
# GVirtuS modes (run.sh also spawns docker per session).
...
BAREMETAL_WORKDIR = os.environ.get(
    "BAREMETAL_WORKDIR", os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if MODE == "baremetal":
    # First-run compile: existing ./dnn_test was linked against the GVirtuS
    # libcudart shim (which exports C++ <string>::size symbols versioned as
    # libcudart.so.12). Re-linking against real libcudart triggers undefined-
    # symbol errors. Solution: build dnn_test_native against real cuDNN+OpenCV
    # the first time, reuse the cached binary on subsequent runs.
    _build = (
        "[ ! -f dnn_test_native ] && nvcc main.cu -o dnn_test_native -g "
        "\\$(pkg-config --cflags --libs opencv4) "
        "-L/usr/local/cuda/lib64 -lcudart -lcublas -lcudnn; "
    )
    # Detect host driver version to bind-mount the matching libnvidia-ml /
    # libcuda. The yolo container was built with a newer driver's user-space
    # libs (mismatching the kernel module on dpu-02), so cudaMallocManaged
    # returns error 803. Mounting the host's versioned .so files restores
    # the driver/runtime ABI contract.
    ...
    _drv_mounts = (
        f' -v /usr/lib/x86_64-linux-gnu/libnvidia-ml.so.{_host_drv_version}:/usr/lib/x86_64-linux-gnu/libnvidia-ml.so.{_host_drv_version}:ro '
        f' -v /usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1:/usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1:ro '
        f' -v /usr/lib/x86_64-linux-gnu/libcuda.so.{_host_drv_version}:/usr/lib/x86_64-linux-gnu/libcuda.so.{_host_drv_version}:ro '
        f' -v /usr/lib/x86_64-linux-gnu/libcuda.so.1:/usr/lib/x86_64-linux-gnu/libcuda.so.1:ro '
        f' -v /usr/lib/x86_64-linux-gnu/libcuda.so:/usr/lib/x86_64-linux-gnu/libcuda.so:ro '
    )
    FRONTEND_CMD = (
        ...
        f"-e LD_LIBRARY_PATH=/usr/local/lib:/usr/local/cuda/lib64:/usr/lib/x86_64-linux-gnu:/usr/local/nvidia/lib64:/usr/local/nvidia/lib "
...
CONFIGS = {
    ...
    "baremetal": "",  # no GVirtuS config in baremetal mode
}
CONFIG = CONFIGS[MODE]

ts = datetime.now().strftime("%Y%m%d_%H%M%S")
out_dir = Path("benchmark_results") / f"opencvdnn_full_metrics_{ts}_{MODE}"
...
def build_env():
    ...
    if MODE == "baremetal":
        # No GVirtuS, no LD_PRELOAD. FRONTEND_CMD already encodes the docker
        # invocation; nothing else to do here.
        return env
...
def run_one(phase, idx):
    ...
    # opencv-dnn does not produce a single output image; it prints "Final Results:"
    # and per-model "Accuracy:" lines after each model finishes. Use those as the
    # success marker instead of yolo's output.jpg check.
    valid_output = (exit_code == 0 and "Final Results" in (output or ""))
