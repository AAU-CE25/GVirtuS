...
MODE = sys.argv[1] if len(sys.argv) > 1 else "ucx"
if MODE not in {"tcp", "rdma", "ucx", "baremetal"}:
    print("Usage: ./benchmark_yolo_p95.py {tcp|rdma|ucx|baremetal}", file=sys.stderr)
    sys.exit(2)
...
NPP_DIR = os.environ.get(
    "NPP_DIR",
    "/home/student.aau.dk/ll33pq/.local/lib/python3.10/site-packages/nvidia/npp/lib"
)

RUNS = int(os.environ.get("RUNS", "50"))
WARMUPS = int(os.environ.get("WARMUPS", "2"))
RUN_TIMEOUT = float(os.environ.get("RUN_TIMEOUT", "3600"))
FRONTEND_CMD = os.environ.get("FRONTEND_CMD", "../run_ucx.sh")

INTERNAL_RUN_ENV = "BENCH_INTERNAL_RUNS"

BAREMETAL_IMAGE = os.environ.get(
    "BAREMETAL_IMAGE",
    "ll33pq/gvirtus-frontend/opencv-yolo:cuda12.6"
)
...
if MODE == "baremetal":
    ...
    _build = (
        "[ ! -f yolo_test_native ] && nvcc main.cu -o yolo_test_native -g "
    ...
    FRONTEND_CMD = (
        ...
        f"--entrypoint bash {BAREMETAL_IMAGE} "
        f"-c \"{_build}BENCH_INTERNAL_RUNS=${{BENCH_INTERNAL_RUNS:-1}} ./yolo_test_native\""
    )


GVIRTUS_LOGLEVEL = os.environ.get("GVIRTUS_LOGLEVEL", "30000")
...
out_dir = Path("benchmark_results") / f"opencvyolo_p95_metrics_{ts}_{MODE}"
...
def delta_net(before, after):
    ...
    for iface in IFACES:
        rx += max(
            0,
            after.get(iface, {}).get("rx", 0)
            - before.get(iface, {}).get("rx", 0)
        )
        tx += max(
            0,
            after.get(iface, {}).get("tx", 0)
            - before.get(iface, {}).get("tx", 0)
        )
...
def extract_inference_latencies_ms(text):
    """
    Extracts every per-detection latency printed by yolo_test.
    ...
    For YOLO, the recommended main.cu should print one:
      BENCH_INTERNAL_RUN_RESULT ...
    per internal run.
    """
    values = []
...
def run_batch(phase, n_runs):
    ...
    # Do not preload GVirtuS shims into Bash itself if a run script is used.
    ...
    valid_output = exit_code == 0 and (
        ...
        or "Detection finished" in output
        or "Detection time:" in output
    ...
    if inf_stats["n"] < 2:
        print(
            ...
            "were parsed from the log. Make yolo_test print one latency per internal run."
        )
