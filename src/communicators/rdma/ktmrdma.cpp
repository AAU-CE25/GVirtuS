...
void ktm_rdma_getaddrinfo(char *node, char *service, struct rdma_addrinfo *hints,
    ...
    if (addrinfoResult == -1) {
        throw std::runtime_error("rdma_getaddrinfo(): error: " + std::string(strerror(errno)));
...
int ktm_rdma_get_send_comp(struct rdma_cm_id *id, struct ibv_wc *wc) {
    ...
    if (returned < 0) {
        throw std::runtime_error("rdma_get_send_comp(): error: " + std::string(strerror(errno)));
    }

    if (wc->status != IBV_WC_SUCCESS) {
        throw std::runtime_error("rdma_get_send_comp(): error: (completion with error) failed status " +
...
int ktm_rdma_get_recv_comp(struct rdma_cm_id *id, struct ibv_wc *wc) {
    ...
    if (returned < 0) {
        throw std::runtime_error("rdma_get_recv_comp(): error: " + std::string(strerror(errno)));
    }
