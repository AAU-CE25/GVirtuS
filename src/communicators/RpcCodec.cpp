...
bool ReadRequest(Communicator *c, am::EnvelopeHeader &header, std::string &routine,
    ...
    if (!valid_header(header)) {
        c->ReleaseFrame();
+        std::fprintf(stderr, "ReadRequest: bad header magic=0x%04x version=%u type=%u (want magic=0x%04x version=%u)\n",
+                     header.magic, header.version, header.message_type,
+                     am::kEnvelopeMagic, am::kEnvelopeVersion);
