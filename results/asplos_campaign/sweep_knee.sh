...
LAMS="${LAMS:-0.55 0.6 0.65 0.7}"
LP=/opt/GVirtuS/examples/llama
MODEL=$LP/models/mistral-7b-q4.gguf
OUT=/home/student.aau.dk/ll33pq/GVirtuS/results/asplos_campaign/llama_slo_knee
mkdir -p "$OUT"
...
for L in $ORD; do
  W=$(python3 -c "print(int(max(90, 120/$L)))")
  LBL="kn_${TAG}_n${N}_l${L}_r${REP}"
  OUTDIR=$OUT SERVERS=$SRVS MODE=open RATE=$L UNIQUE=1 SEED=$REP LABEL=$LBL \
    CAMPAIGN_ID=slo_knee WINDOW=$W WARM=10 NPRED=128 REQ_TIMEOUT=25 \
    python3 $HOME/bench.py 2>&1 | tail -1
done
echo "KN_${TAG}_N${N}_REP${REP}_DONE"
