...
2. GPU RMA 1.78–1.80× host RMA at 1 GiB
3. cuDF 13.6–15.4% over the same protocol with a host bounce
4. The TinyLlama 2×2 (async versus graphs)
5. Multi-tenant throughput 1.37× over default native
...
| GPU RMA 1.78–1.80× host RMA | competing | **evaluation, folded into the crossover figure** | it *is* the crossover, at one size |
| cuDF 13.6–15.4% | competing | **evaluation, the end-to-end case** | keep: it is the workload that exercises the lead |
| TinyLlama 2×2 | competing | **evaluation** | orthogonality already qualified; not an opening claim |
| Multi-tenant 1.37× | competing | **evaluation, with the MPS parity in the same sentence** | see §4 |
| ~461 MiB/tenant | competing | **evaluation** | see §4 |
| Capacity under an SLO (+17.4%) | new | **evaluation, and it replaces the saturated goodput headline** | it is the serving claim |
...
- **Multi-tenant throughput.** *"1.37× over default native, and 97–99% — parity — against
  MPS-configured native."* Quoting only the first is a straw man; quoting only the second
  discards a practical result. **Never ×1.42**: that was the maximum of three runs; the mean is
  ×1.37.
...
- **Fairness.** *"Serving fairness is statistically equivalent to native; under equal fixed
  work it is not — one tenant runs at its single-client rate while another is 4.87× slower."*
...
- **Any Jain index over per-tenant throughput.** Demand imbalance reaches 7.0× from the arrival
...
- [ ] No ×1.42 anywhere. No saturated goodput quoted as a rate without its window.
