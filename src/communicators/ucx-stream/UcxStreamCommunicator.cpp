#include "UcxStreamCommunicator.h"
...
using gvirtus::communicators::UcxStreamCommunicator;
...
UcxStreamCommunicator::UcxStreamCommunicator(const std::string &hostname, uint16_t port)
    : mHostname(hostname), mPort(port) {}

UcxStreamCommunicator::UcxStreamCommunicator(ucp_context_h ctx, ucp_worker_h worker, ucp_ep_h ep)
    : mContext(ctx), mWorker(worker), mEndpoint(ep), mOwnsContext(false) {}

UcxStreamCommunicator::~UcxStreamCommunicator() { Close(); }

// ============================================================================
// Pre-registered staging buffer management
// ============================================================================

void UcxStreamCommunicator::ensureStaging(size_t needed) {
...
void UcxStreamCommunicator::freeStaging() {
...
void UcxStreamCommunicator::initContext() {
    ucp_params_t params{};
    ...
    if (status != UCS_OK) {
        throw std::runtime_error(std::string("UcxStreamCommunicator: ucp_init failed: ") +
                                 ucs_status_string(status));
    }
    mOwnsContext = true;
}

void UcxStreamCommunicator::initWorker() {
    ...
    if (status != UCS_OK) {
        throw std::runtime_error(std::string("UcxStreamCommunicator: ucp_worker_create failed: ") +
                                 ucs_status_string(status));
    }
}
...
void UcxStreamCommunicator::epErrCallback(void *arg, ucp_ep_h /*ep*/, ucs_status_t status) {
    auto *self = static_cast<UcxStreamCommunicator *>(arg);
...
void UcxStreamCommunicator::wireupServer(ucp_ep_h ep) {
    char ack = 'A';
    ...
    if (ready != 'R') {
        throw std::runtime_error("UcxStreamCommunicator: invalid wireup READY byte");
    }
}

void UcxStreamCommunicator::wireupClient(ucp_ep_h ep) {
    char ack = 0;
    stream_recv_all(mWorker, ep, &ack, 1);
    if (ack != 'A') {
        throw std::runtime_error("UcxStreamCommunicator: invalid wireup ACK byte");
    }
    char ready = 'R';
...
void UcxStreamCommunicator::listenerCallback(ucp_conn_request_h conn_request, void *arg) {
    auto *self = static_cast<UcxStreamCommunicator *>(arg);
    self->mConnReq.conn_req = conn_request;
    self->mConnReq.ready.store(true);
}

void UcxStreamCommunicator::Serve() {
    ...
    if (mHostname.empty() || mHostname == "0.0.0.0") {
        ...
        if (inet_pton(AF_INET, mHostname.c_str(), &listen_addr.sin_addr) != 1) {
            ...
            if (!ent) {
                throw std::runtime_error("UcxStreamCommunicator: Can't resolve hostname '" + mHostname +
                                         "'");
            }
    ...
    listener_params.conn_handler.arg = const_cast<UcxStreamCommunicator *>(this);

    ucs_status_t status = ucp_listener_create(mWorker, &listener_params, &mListener);
    if (status != UCS_OK) {
        throw std::runtime_error(std::string("UcxStreamCommunicator: ucp_listener_create failed: ") +
                                 ucs_status_string(status));
    }
...
const gvirtus::communicators::Communicator *const UcxStreamCommunicator::Accept() const {
    ...
    if (status != UCS_OK) {
        throw std::runtime_error(
            std::string("UcxStreamCommunicator::Accept: ucp_worker_create failed: ") +
            ucs_status_string(status));
    }
    ...
    auto *client = new UcxStreamCommunicator(mContext, client_worker, nullptr);
    ...
    if (status != UCS_OK) {
        ...
        delete client;
        throw std::runtime_error(std::string("UcxStreamCommunicator::Accept: ucp_ep_create failed: ") +
                                 ucs_status_string(status));
    }
...
void UcxStreamCommunicator::Connect() {
    ...
    if (status != UCS_OK) {
        throw std::runtime_error(std::string("UcxStreamCommunicator: ucp_ep_create (connect) failed: ") +
                                 ucs_status_string(status));
    }
...
size_t UcxStreamCommunicator::Read(char *buffer, size_t size) {
...
size_t UcxStreamCommunicator::Write(const char *buffer, size_t size) {
...
void UcxStreamCommunicator::Sync() {
    if (!mEndpoint) return;
    OpState state;
    ucp_request_param_t param{};
...
void UcxStreamCommunicator::Close() {
    if (mEndpoint) {
...
extern "C" std::shared_ptr<UcxStreamCommunicator> create_communicator(
    ...
    if (!ucx_end) {
        throw std::runtime_error("UcxStreamCommunicator: expected Endpoint_Ucx");
    }
    return std::make_shared<UcxStreamCommunicator>(ucx_end->address(), ucx_end->port());
}
