...
static void celda(size_t nb) {
    ...
    comprueba("lanzamiento 1 (origen 0x11)", primer_byte(d), 0x11);
    std::memset(h, 0x22, nb);
    cudaGraphLaunch(ge, s); cudaStreamSynchronize(s);
    comprueba("lanzamiento 2 (origen 0x22)", primer_byte(d), 0x22);
    comprueba("lanzamiento 2, ultimo byte",  byte_en(d, nb - 1), 0x22);
    std::memset(h, 0x33, nb);
    cudaGraphLaunch(ge, s); cudaStreamSynchronize(s);
    comprueba("lanzamiento 3 (origen 0x33)", primer_byte(d), 0x33);
limpia:
...
static void multinodo(size_t nb) {
    std::printf("RMA,multinodo 3 x %zu bytes\n", nb);
    ...
    if (e == cudaSuccess && cudaGraphInstantiate(&ge, g, nullptr, nullptr, 0) == cudaSuccess) {
        ...
        cudaGraphLaunch(ge, s); cudaStreamSynchronize(s);
        for (int i = 0; i < 3; ++i) {
            char nom[48]; std::snprintf(nom, sizeof nom, "nodo %d tras refrescar", i);
...
int main() {
    ...
    std::printf("RMA,RESULTADO fallos=%d\n", fallos);
    return fallos == 0 ? 0 : 1;
}
