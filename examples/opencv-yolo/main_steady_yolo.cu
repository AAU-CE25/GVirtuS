...
int main() {
    try {
        ...
        int inputSize = getenv_int("YOLO_INPUT_SIZE", 416);

        if (imagePath.empty()) {
            imagePath = first_file_with_ext({".jpg", ".jpeg", ".png"});
            if (imagePath.find("output") != string::npos) {
                imagePath = "";
                for (const auto& entry : fs::recursive_directory_iterator(".")) {
                    if (!entry.is_regular_file()) continue;
                    string p = entry.path().string();
        ...
        if (!modelPath.empty()) {
            ...
            if (configPath.empty()) configPath = first_file_with_ext({".cfg"});
            string weightsPath = getenv_or("YOLO_WEIGHTS", "");
            if (weightsPath.empty()) weightsPath = first_file_with_ext({".weights"});
