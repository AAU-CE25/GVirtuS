...
| # | item | status |
|---|---|---|
| 5 | second hardware validation | **not done** |
| 6 | attribute the ~460 MiB | **CLOSED, with a mechanism** |
| 7 | harmful epoch-collision demonstration | **still open, and now with the reason** |
...
**That is not evidence the guard is unnecessary. The experiment is inert and the logs prove
it:** the epoch changes but the indices **renumber**, `[0-7]` -> `[9-16]`, and with renumbering
there is no collision. `ack_epoch_dropped=0`, so the guard never fired and the ablation had
nothing to disable.

**The lesson generalises:** a bench that grows the pool triggers a *rebuild*, and a rebuild
renumbers. The dangerous state needs a **re-advertisement without a rebuild** -- which cuDF
produces naturally and which changing transfer sizes cannot produce by construction. Two
concrete routes to close it are named in `N7_EPOCH.md`.

## 5 -- not done, and why

The plan was sound: dpu-02 has its own L40S on a **different driver** (560.35.05 against
580.95.05), so reversing the roles is a genuine second configuration. It was not run because
dpu-02's GPU is occupied by the capacity sweep until ~07:10, and standing up a backend there
plus a frontend on dpu-01 is new infrastructure that should not be left unattended untested.
