...
| tarball | size | what it is | enough for |
|---|---:|---|---|
| `paper_docs_2026-08-03.tar.gz` | 4.7 MB | 25 markdown + 32 PDF + `figures/` + the aggregated CSVs | **reading the results** |
| `paper_raw_2026-08-03.tar.gz` | 0.5 MB | the `results/asplos_campaign/` trees the documents cite | **re-deriving every number** |
| `paper_2026-08-03.tar.gz` | 36.1 MB | the whole `paper/` folder, including `cudf_etl/` (190 MB raw) | archival |

The raw tarball exists because the first two packages shipped the documents and the aggregated
CSVs but **not** the logs underneath them -- so a reader could check the arithmetic and not the
provenance. It carries:

| tree | files | what it backs |
|---|---:|---|
| `llama_slo_sweep_v2/` | 223 | the corrected SLO sweep (§3c of `LLAMA-7B_RESULTS.md`) |
| `sched/` | 81 | the scheduler traces, `GVS_SCHED_TRACE` (`N1_SCHEDULER.md` §2-3) |
| `sched_n1/` | 2 | the fair-dispatch A/B, 33 cohorts (`N1_SCHEDULER.md` §5) |
| `epoch_n7/` | 30 | the epoch 2x2 plus the fifth cell, with full client logs (`N7_EPOCH.md`) |
| `epoch/` | 13 | the earlier epoch runs, kept because §3 of `N7_EPOCH.md` reinterprets them |
| `memoria/` | 5 | `mem_footprint.csv` (four arms), `ctx_probe_results.csv`, `ctx_probe.cu` (§2 of `LLAMA-7B_RESULTS.md`) |
| `fairness/` | 32 | every per-tenant and per-cohort table of the audit |

**Still not packaged**, and named so it is not mistaken for complete: the workload trees under
`docs/gusto_raw_2026-08-02/` and `results/pool_provisioning*/` that the fallback-rate aggregate
in `IMPLEMENTATION_COVERAGE.md` §3 sums over (620 teardown records, 14.78 M operations). Those
are large and unchanged since the previous package.
