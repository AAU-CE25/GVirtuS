...
int main(){
    ...
    CASO("cudaStreamQuery(OTHER stream)", (void)cudaStreamQuery(g_otro));
    CASO("cudaMemcpyAsync D2H to PAGEABLE dst on OTHER stream",
         cudaMemcpyAsync(g_pag,g_dev,8<<20,cudaMemcpyDeviceToHost,g_otro));
    CASO("cudaMemcpyAsync D2H to PINNED dst on OTHER stream",
         cudaMemcpyAsync(g_pin,g_dev,8<<20,cudaMemcpyDeviceToHost,g_otro));
    CASO("cudaLaunchHostFunc(OTHER stream)", {
    ...
    std::printf("\nD2H 8MiB to pageable: call returned in %ldus, following sync %ldus -> %s\n",
                us(t0,t1), us(t1,t2), us(t0,t1) > 10*us(t1,t2) ? "BLOCKS (free wait)" : "does NOT block");
    return 0;
}
