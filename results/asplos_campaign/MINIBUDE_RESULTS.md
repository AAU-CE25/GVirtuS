...
> **Corrected 2026-08-01.** The previous version of this table mixed sources and statistics,
> and its caption ("mean of 5 reps") described none of its columns. See
> `CORRECTIONS_2026-08-01.md`. What was checked, cell by cell, against the raw data that ships:
>
> - **The GVirtuS column was only half updated.** `N=4` and `N=8` **do** reproduce the current
>   raw data (12.748 against 12.78 published; 24.888 against 24.91), so those two cells **were**
>   re-measured. `N=1` and `N=2` reproduce **nothing** that ships (3.679 against 3.79;
>   6.655 against 6.79) and remain the old measurement. That is why the banner ("the GVirtuS
>   numbers were redone") and this section ("the column is still pre-cutoff") appeared to
>   contradict each other: **both were true, for different cells**, and the table did not say so.
> - **Each column used a different statistic.** `baremetal` without MPS was the **median of
...
>   **sd = 1.613 s** at N=8, and with MPS **sd = 0.008 s** -- two hundred times smaller. Taking
>   the median in the dispersed column and the mean in the narrow one inflates the non-MPS
>   baseline, which is precisely the comparison the conclusion rests on.
> - **`baremetal_mps.csv` carries `sys=baremetal` in its rows.** Grouping by that column
